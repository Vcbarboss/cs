...
export default function SelectField2(props) {
    ...
    const handleChange = (e, index) => {
        ...
        setIsVisible(false);
        props.change(e, index);
    };

    const handleSelect = (item) => {
        let aux = props.selectedList;
        if (!aux.includes(item.answer)) {
            aux.push(item.answer);
            props.change(aux);
            setSelectedCount(selectedCount + 1);
        } else if (aux.length > 0) {
            aux.splice(aux.indexOf(item.answer), 1);
            props.change(aux);
            setSelectedCount(selectedCount - 1);
        }
    };
            ...
            {props.multiple ?
                <>
                    <View style={{
                        ...
                        {props.icon && <Icon style={[styles.icon, {marginBottom: 15}]} name={props.icon} size={24}
                ...
                :
                <View>
                    <FieldForm icon={props.icon && props.icon} value={selected} label={props.label && props.label}
                    ...
                    <ScrollView>
                        {props.list.map((item, index) => (
                                <TouchableOpacity style={[styles.list, {
                                    borderBottomWidth: index + 1 === props.list.length ? 0 : 1,
                                    backgroundColor: props.multiple && props.selectedList?.includes(item.answer) ? '#f7f7f7' : Colors.white
                                }]} key={index} onPress={() => {
                                    props.multiple ? handleSelect(item) : handleChange(item, index)
                                }}>
                                    <Icon size={20} name='chevron-forward-outline' color='#517fa4'/>
                                    <Text style={{
                                        fontWeight: props.multiple && props.selectedList?.includes(item.answer) ? 'bold' : '400',
                                        color: props.multiple && props.selectedList?.includes(item.answer) ? Colors.secondary : Colors.dark
                                    }}> {item.answer} </Text>
                                </TouchableOpacity>
                            )
                        )}
