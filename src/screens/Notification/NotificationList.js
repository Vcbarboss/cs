...
import {
    ...
    Dimensions,
    Text,
    Modal, SafeAreaView, Platform, KeyboardAvoidingView, Animated, Keyboard,
} from "react-native";
...
export function NotificationList({navigation}) {
    ...
    const getData = async (more) => {
        ...
        try {
            const res = await api.get(`app/notification/paginate?page=${paginate.current.page + 1}&limit=10&order_field=created_at&order_type=DESC&search_list=${filterAux.value}`);
